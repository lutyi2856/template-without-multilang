...
import { getOurWorkWord } from "@/lib/utils/our-work-plural";
...
interface OurWorksFiltersProps {
  categories: PromotionServiceCategory[];
  worksCount?: number;
}

/**
 * Фильтры архива наших работ: категории (service_categories).
 * Показываются только категории, у которых есть работы.
 * Фильтр скрыт, если вариантов 1 или меньше.
 * Desktop: чипы (flex-wrap). Mobile: кнопка «Фильтры» + Sheet.
 */
export function OurWorksFilters({
  categories,
  worksCount,
}: OurWorksFiltersProps) {
  ...
  const updateFilters = (updates: { category?: string | null }) => {
    ...
    if (typeof window !== "undefined") {
      sessionStorage.setItem("our-works-scroll-y", String(window.scrollY));
    }
    startTransition(() => {
        ...
        {hasActiveFilters && activeCategoryName && (
          <div className="mt-4 flex flex-wrap gap-2">
            <span className={cn(chipBase, chipActive)}>
              {worksCount != null
                ? `${activeCategoryName} ${worksCount} ${getOurWorkWord(worksCount)}`
                : activeCategoryName}
            </span>
          </div>
        )}
